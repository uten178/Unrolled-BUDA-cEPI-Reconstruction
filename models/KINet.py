...
def KINet(data_consistency_type,
    ...
    if use_DL_regularizer:
        ...
        reg_model_i = UNet2D(im_size=num_rows, kernel_size=3, num_out_chan_highest_level=64,
        ...
        reg_model_k = UNet2D(im_size=num_rows, kernel_size=3, num_out_chan_highest_level=64,
                           depth=5, num_chan_increase_rate=2, activation_type=activation_type,
                           dropout_rate=0.05, SEPARABLE_CONV=False, SKIP_CONNECTION_AT_THE_END=True,
                           num_input_chans=2, num_output_chans=2)

        if pretrained_reg_net_path is not None:
            reg_model_i.load_weights(pretrained_reg_net_path)
            reg_model_k.load_weights(pretrained_reg_net_path) # Better to load different weights though ...
